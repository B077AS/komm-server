...
import com.kommserver.websocket.messages.payloads.PermResetRoleRequestPayload;
...
public class PermResetRoleRequestHandler implements HubInboundMessageHandler {
    ...
    @Override
    public WsMessageType getType() { return WsMessageType.PERM_RESET_ROLE_REQUEST; }

    @Override
    public void handle(JsonObject payload) {
        PermResetRoleRequestPayload req = gson.fromJson(payload, PermResetRoleRequestPayload.class);
        try {
            ServerMember.Role role = ServerMember.Role.valueOf(req.getRole().toUpperCase());
            permissionService.resetRoleToDefault(req.getServerId(), req.getUserId(), role);
            broadcastPermissionsUpdated(req.getServerId());
            hubMessageSender.send(WsMessageType.PERM_RESET_ROLE_RESPONSE, PermProxyResponsePayload.builder()
                    .correlationId(req.getCorrelationId()).success(true).build());
        } catch (Exception e) {
            log.error("PERM_RESET_ROLE_REQUEST failed: {}", e.getMessage());
            hubMessageSender.send(WsMessageType.PERM_RESET_ROLE_RESPONSE, PermProxyResponsePayload.builder()
